...
function OnFilterscriptInit()
	...
	--Gates
	movement = CreateMovement(MovementType.TYPE_GATE,MovementStatus.STATUS_CLOSED,"EVT_GATE_LARGE_01.3DS","WOM_COLONY.ZEN");
	SetMovementCoordinationAsOpened(movement,-3048.2077636719, -332.78945922852, 1595.6101074219,0,155,0);
	SetMovementCoordinationAsClosed(movement,-3048.2077636719, 432.78945922852, 1595.6101074219,0,155,0);
	...
	--Doors
	movement = CreateMovement(MovementType.TYPE_DOOR,MovementStatus.STATUS_CLOSED,"DOOR_NW_RICH_01.MDS","WOM_COLONY.ZEN");
	SetMovementCoordinationAsOpened(movement,-2006.1151123047, 247.83042907715, -872.5185546875,0,179,0);
	SetMovementCoordinationAsClosed(movement,-1900.4432373047, 244.27890014648, -1035.0516357422,0,89,0);
...
function OnPlayerConnect(playerid)
	...
	SendPlayerMessage(playerid,0,255,0,"(PL)  Jeњli jesteњ w pobliїu bramy lub drzwi moїesz uїyж /open i /close");]]
end
...
function OnPlayerHasItem(playerid, item_instance, amount, equipped, checkid)
	...
	if item_instance == "NULL" then
			GameTextForPlayer(playerid,50,5500,string.format("%s","У Вас нет ключей!"),"FONT_OLD_10_WHITE_HI.TGA",255,0,0,3000);
			else
	local movement = GetNearestMovement(playerid);
		if movement ~= nil then
				...
				if movement.status == MovementStatus.STATUS_CLOSED or movement.status == MovementStatus.STATUS_MOVING_TO_CLOSE then
					...
					GameTextForPlayer(playerid,50,5500,string.format("%s","Открыто!"),"FONT_OLD_10_WHITE_HI.TGA",0,255,0,3000);
				elseif movement.status == MovementStatus.STATUS_OPENED or movement.status == MovementStatus.STATUS_MOVING_TO_OPEN then
					...
					GameTextForPlayer(playerid,50,5500,string.format("%s","Закрыто!"),"FONT_OLD_10_WHITE_HI.TGA",255,255,0,3000);
				end
		end
	end
	end
...
--[[function OnPlayerCommandText(playerid, cmdtext)
	...
	if cmdtext == "/открыть" then
	
		local movement = GetNearestMovement(playerid);
		if movement ~= nil then
			OpenMovement(movement);
			GameTextForPlayer(playerid,2500,4000,"Открыто!","Font_Old_20_White_Hi.TGA",0,255,0,3000);
		end
	
	elseif cmdtext == "/закрыть" then
	
		local movement = GetNearestMovement(playerid);
		if movement ~= nil then
			CloseMovement(movement);
			GameTextForPlayer(playerid,2500,4000,"Закрыто!","Font_Old_20_White_Hi.TGA",255,255,0,3000);
		end
	end
end]]
