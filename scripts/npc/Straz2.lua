...
function Straz2()
	local npc = CreateNPC(GetNewNPCName("Патрульный(2)"));
	local skin = random(6)
	if skin == 0 then
...
if(TA_FUNC(playerid, 22, 0, 06, 0))then
          ...
          AI_GOTO(playerid, "NW_CITY_HABOUR_02_B"); --Since version 0.11 AI_GOTO works with way- and freepoints.
     ...
     elseif(TA_FUNC(playerid, 06, 0, 06, 30))then
          ...
          AI_GOTO(playerid, "NW_CITY_HABOUR_KASERN_CENTRE_02");
		  SetPlayerWeaponMode(playerid,0);
	elseif(TA_FUNC(playerid, 06, 30, 07, 0))then
     ...
     elseif(TA_FUNC(playerid, 07, 0, 22, 0))then
          AI_ClearStates(playerid);
          AI_SETWALKTYPE(playerid, 0);
          AI_GOTO(playerid, "NW_CITY_MERCHANT_PATH_24");
		  SetPlayerWeaponMode(playerid,0);
