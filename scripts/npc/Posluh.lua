...
function Posluh()
	local npc = CreateNPC(GetNewNPCName("Послушник"));
	local skin = random(6)
	if skin == 0 then
	...
	if armor == 0 then
	EquipArmor(npc,"ITAR_NOV_L");
	EquipMeleeWeapon(npc,"ItMw_1H_Bau_Axe");
	elseif armor == 1 then
	EquipArmor(npc,"ITAR_NOV_L");
	EquipMeleeWeapon(npc,"ItMw_1H_Mace_L_03");
	elseif armor == 2 then
	EquipArmor(npc,"ITAR_NOV_L");
	EquipMeleeWeapon(npc,"ItMw_1H_Vlk_Mace");
	end
