...
function AI_ALIGNTOWP(_playerid, _waypoint)
	...
	if(lAiState.waypoint == nil)then
		print("AI_ALIGNTOWP: Couldn't find the Waypoint ".._waypoint.." in World:"..AI_NPCList[_playerid].GP_World.." Player: ".._playerid);
		return;
	end
	table.insert(AI_NPCList[_playerid]["STATES"],lAiState);
end
...
function ai_follow_func(_aistate)
	local px,py,pz = GetPlayerPos(_aistate.followingPlayer);
	local dist = getDistance(_aistate.endWP.x, _aistate.endWP.y, _aistate.endWP.z, px, py, pz);
	if(dist < 300)then--Wenn kleiner als 500, soll er aufhören zu folgen!
		AI_NPCList[_aistate.player].LASTWP = _aistate.endWP.name;
		PlayAnimation(_aistate.player,"S_LGUARD");--Stoppen!
		table.remove(AI_NPCList[_aistate.player]["STATES"], 1);
		return;
	end
	local ox,oy,oz = GetPlayerPos(_aistate.player);
	local angle = getAngle(ox, oz, px, pz);
	SetPlayerAngle(_aistate.player,angle);
	
	if(GetDistancePlayers(_aistate.followingPlayer, _aistate.player) < 250) then
		PlayAnimation(_aistate.player,"S_LGUARD");--stopp!
	else
		--PlayAnimation(_aistate.player,"S_WALKL");--start!
		run(_aistate.player);
	end
end
...
function AI_PLAYANIMATION(_playerid, _anim)
	AI_Set(_playerid);
...
function ai_waitforchoise_func(_aistate)
	if(_aistate.start) then
		...
		_aistate.playerid = AI_NPCList[_aistate.player].DIA_TARGET;
